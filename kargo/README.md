...
| 6 | `argocd-update` | Selects ArgoCD apps by label `stage=<stage>` and nudges them to sync (all clusters for that stage). |
...
- `${{ imageFrom(vars.backendImage).Tag }}` pulls the tag out of the Freight
  being promoted.
- The `argocd-update` step uses a **label selector** (`matchLabels: {stage:
  ${{ ctx.stage }}}`) instead of naming apps. The ApplicationSet stamps that
  `stage` label on every app it generates, so this matches all clusters for the
  stage and never needs editing when clusters change (see `../argocd/README.md`).
...
## 5. Apply the Kargo manifests

Apply these **first** — the Project creates the `authenticwrite` namespace that
everything else (including the Git credential in Section 6) lives in.

```bash
# Apply all Kargo manifests (use the kargo CLI, not kubectl, for these).
# This creates the Project (→ the authenticwrite namespace), Warehouse,
# the 3 Stages, and the PromotionTask.
kargo apply -f ./kargo/

# Confirm
kubectl get project,warehouse,stages,promotiontasks -n authenticwrite

# Make sure Freight was discovered (images must be public!). Discovery takes a
# few seconds; re-run until a Freight row appears. No credential is needed for
# discovery — only the push during a promotion needs one (Section 6).
kubectl get freight -n authenticwrite

# See the image tags inside a Freight (swap in the Freight name from above)
kubectl get freight <FREIGHT_NAME> -n authenticwrite \
  -o jsonpath='{range .images[*]}{.repoURL}  ->  {.tag}{"\n"}{end}'
```

---

## 6. Add Git write-credentials & run a promotion

The project namespace now exists, so the credential can be registered against it.
...
Now promote via the **UI** (http://localhost:3100 → authenticwrite project →
click the target icon on the `dev` stage → pick Freight → confirm), then repeat
`dev → staging → prod`. Or promote/watch from the CLI:

```bash
# Promote a specific Freight into the dev stage
kargo promote --project=authenticwrite --stage=dev --freight=<FREIGHT_NAME>

# Watch promotions run
...
> ⚠️ **`argocd-update` needs the ArgoCD apps to exist.** The PromotionTask's last
> step selects Applications by the label `stage=<stage>` and nudges them to sync.
> Those apps (and that label) are generated by the ArgoCD **ApplicationSet**
> (`../argocd/`). If you run a promotion *before* applying that ApplicationSet,
> the git-clone → commit → **push** still succeed (the tag bump lands in Git),
> but the `argocd-update` step matches no apps. Apply `../argocd/` to complete
> the loop.
...
ArgoCD syncs charts/authenticwrite (with that values file) to BOTH clusters
        │
        ▼
New pods running in authenticwrite-<stage> on cluster 1 + cluster 2
```
