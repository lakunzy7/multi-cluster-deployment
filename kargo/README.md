...
> step nudges Applications `authenticwrite-<stage>-local` and `-gke` to sync.
> Those are generated by the ArgoCD **ApplicationSet** (`../argocd/`). If you run
> a promotion *before* applying that ApplicationSet, the git-clone → commit →
> **push** still succeed (the tag bump lands in Git), but the `argocd-update`
> step has no apps to sync. Apply `../argocd/` to complete the loop.
